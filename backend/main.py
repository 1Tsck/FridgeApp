...
async def admin_page(request: Request, filter: str = Query("", alias="filter"), msg: str = Query("", alias="msg"), user=Depends(dependencies.require_admin)):
...
async def create_item_type(add_item_name: str = Form(...), add_item_desc: str = Form(...), user=Depends(dependencies.require_admin)):
...
async def add_fridge_item(add_item_item_type_id: str = Form(...), add_item_quantity: float = Form(...), add_item_unit: str = Form(...), add_photo: UploadFile | None = File(None), add_expiry_date:str | None = Form(None), user=Depends(dependencies.get_current_user)):
...
async def add_cart_item(add_item_item_type_id: str = Form(...), add_item_quantity: float = Form(...), add_item_unit: str = Form(...), user=Depends(dependencies.get_current_user)):
...
async def add_to_cart(item_type_id: str = Form(...), quantity: float = Form(...), unit: str = Form(...), user=Depends(dependencies.get_current_user)):
...
async def update_item_type(item_type_id: str = Form(...), name: str = Form(...), description: str = Form(''), user=Depends(dependencies.require_admin)):
...
async def update_fridge_item(item_id: str = Form(...), type_name: str = Form(...), quantity: float = Form(...), unit: str = Form(...), photo: UploadFile | None = File(None), expiry_date: str | None = Form(None), user=Depends(dependencies.get_current_user)):
...
async def update_cart_item(cart_id: str = Form(...), quantity: float = Form(...), unit: str = Form(...), user=Depends(dependencies.get_current_user)):
...
async def delete_item_type(item_type_id: str = Form(...), user=Depends(dependencies.require_admin)):
    err = crud.delete_item_type(item_type_id)
    url = prepareUrl('/admin', err, 'Item type deleted successfully!')
    return RedirectResponse(url, status_code=303)

@app.post("/fridge/item/delete")
async def delete_fridge_item(type_name: str = Form(...), item_id: str = Form(...), user=Depends(dependencies.get_current_user)):
...
async def delete_cart_item(cart_id: str = Form(...), user=Depends(dependencies.get_current_user)):
